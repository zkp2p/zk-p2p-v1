...
// Extract the market maker user ID from the email
...
template VenmoMmRegex (msg_bytes) {
    ...
    final_state_sum[0] <== states[0][14];
    for (var i = 1; i <= num_bytes; i++) {
        final_state_sum[i] <== final_state_sum[i-1] + states[i][14];
    }
